...
async function show() {
  ...
  let res = await fetch(
    `https://api.coursera.org/api/courses.v1?q=search&query=${input}&includes=instructorIds,photoUrl,description&fields=instructorIds,photoUrl,description,partnerLogo,certificates,partnerIds,startDate,workload,specializations,domainTypes`
  );
...
async function fillPreviousSearched() {
  ...
  let res = await fetch(
    `https://api.coursera.org/api/courses.v1?q=search&query=${q}&includes=instructorIds,photoUrl,description,largeIcon&fields=instructorIds,photoUrl,description,partnerLogo,certificates,startDate,partnerIds,workload,specializations,domainTypes`
  );
  let data = await res.json();
...
for (let i = 1; i < hoverdivs.length; i++) {
  ...
  hoverdivs[i].onmouseover = async () => {
    ...
    if (i >= 6) {
      ...
      let res = await fetch(
        `https://api.coursera.org/api/courses.v1?q=search&query=${q[0]}&includes=instructorIds,photoUrl,partnerIds,description,largeIcon,shortName,location&fields=instructorIds,partnerIds ,location,photoUrl,description,partnerLogo,certificates,startDate,workload,specializations,domainTypes`
      );
      let data = await res.json();
...
logout.addEventListener("click", () => {
  ...
  jff.style.display = "block";
  login.style.display = "block";

  bellsign.style.display = "none";
  hiddenfig.style.display = "none";
